...
export function extractRawVariantPrintingId(variantRef) {
  const rawValue = String(variantRef ?? "").trim();
  if (!rawValue) return null;

  const parts = rawValue.split("::");
  const isCanonicalRaw = parts.length === 2 && parts[1] === "RAW";
  const isProviderHistoryRaw = parts.length === 3 && parts[2] === "RAW";
  if (!isCanonicalRaw && !isProviderHistoryRaw) return null;

  const printingId = parts[0]?.trim() ?? "";
  return printingId || null;
}

export function isRawHistoryVariantRefForPrinting(variantRef, printingId) {
  const normalizedPrintingId = String(printingId ?? "").trim();
  if (!normalizedPrintingId) return false;
  return extractRawVariantPrintingId(variantRef) === normalizedPrintingId;
}
...
export function parseVariantRef(variantRef) {
  ...
  const parts = rawValue.split("::");
  if (parts.length === 2 && parts[1] === "RAW" && parts[0]) {
    return {
      printingId: parts[0],
      mode: "RAW",
      provider: null,
  ...
  if (
    parts.length !== 3
    || !parts[0]
    || !GRADED_PROVIDERS.has(parts[1])
    || !["7_OR_LESS", "8", "9", "9_5", "10", "10_PERFECT"].includes(parts[2])
  ) {
    return null;
  }
