...
const WEIGHTS = {
  set_number_match: 5000,
  exact_name_match: 2000,
  alias_match: 900,
  exact_norm_match: 400,
  prefix_match: 100,
...
export function computeSearchSignals(row, aliasNorms, query) {
  ...
  const alias_match = aliasNorms.some(
    (aliasNorm) => aliasNorm.includes(qNorm) || matchesAllTokens(aliasNorm, normalized.tokens),
  );

  const matchedTokenCount = normalized.tokens.filter((token) => rowNorm.includes(token)).length;
