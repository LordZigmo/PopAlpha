...
async function fetchAllProviderCardMapRows(select: string): Promise<ProviderCardMapSummaryRow[]> {
...
export async function loadScrydexSetFootprints(): Promise<ScrydexSetFootprint[]> {
  const supabase = dbAdmin();
  const [providerCardMapRows, providerSetMapRows] = await Promise.all([
    fetchAllProviderCardMapRows("provider_set_id, provider_card_id, mapping_status"),
    (async () => {
  ...
  for (const [providerSetId, bucket] of footprintBySet.entries()) {
    ...
    footprints.push({
      ...
      providerCardCount,
      matchedCardCount,
      dailyCaptureRequests: Math.max(1, Math.ceil(providerCardCount / 100)),
      historyBackfillRequests: matchedCardCount,
      historyBackfillCredits: matchedCardCount * SCRYDEX_HISTORY_CREDITS_PER_CARD,
      priorityReasons: [],
    });
  }
...
async function loadScrydexCardHistoryTargets(params: {
  ...
  const targets = [...byCardId.values()].sort((left, right) => left.providerCardId.localeCompare(right.providerCardId));
  if (typeof params.maxCards === "number" && Number.isFinite(params.maxCards) && params.maxCards > 0) {
    return targets.slice(0, Math.floor(params.maxCards));
  }
  return targets;
}

async function loadExistingSnapshotDayKeys(variantRefs: string[], sinceIso: string): Promise<Set<string>> {
  const supabase = dbAdmin();
...
export async function backfillScrydexPriceHistoryForSet(opts: {
  ...
  const targets = await loadScrydexCardHistoryTargets({
    providerSetId,
    maxCards: opts.maxCards ?? DEFAULT_HISTORY_MAX_CARDS_PER_RUN,
  });
  const variantRefs = normalizeStringList(
    targets.flatMap((target) => target.variants.map((variant) => variant.historyVariantRef)),
  );
  const sinceIso = new Date(Date.now() - (days * 24 * 60 * 60 * 1000)).toISOString();
  const existingSnapshotDayKeys = await loadExistingSnapshotDayKeys(variantRefs, sinceIso);
    ...
    for (const target of targets) {
      ...
      for (const historyDay of historyDays) {
        ...
        for (const variant of target.variants) {
          ...
          if (existingSnapshotDayKeys.has(existingDayKey)) {
          ...
          existingSnapshotDayKeys.add(existingDayKey);
