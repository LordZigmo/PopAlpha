 ...
 * catalog matching. Snkrdunk's name format has more variation than
 * initially thought — empirical sampling (50 IDs from sub-sitemap 0)
 * found at least 6 real-world shapes:
 *
 *   "Charizard VMAX HR: PROMO[S-P 104](S-P Promotional cards)"
 *   "Blastoise: Old Back/PROMO[PMCG-P No.009](PMCG-P Promotional cards)"
 *   "Charizard VMAX RR [074/073](Sword & Shield "Champion's Path")"
 *   "Pikachu [025/214](Sun & Moon Promo)"
 *   "Mewtwo V SR: SA[S10b 074/071](Enhanced Expansion Pack "Pokemon GO")"
 *   "Shiinotic C [SM7b 037/050](Enhanced Expansion Pack "Fairy Rise")"
 *   "Radiant Charizard K[S10b 011/071](Enhanced Expansion Pack "Pokemon GO")"
 *
 * Rather than maintain a regex per format, we use ONE generic regex
 * that anchors on the trailing `[<bracket>](<paren>)` structure:
 *
 *   <leading-name> [optional ":" + designation] [bracket-content] (paren-content)
 *
 * Then split each captured group:
 *   - leading-name → pokemonName
 *   - designation (after ":") → designation (PROMO, SA, Old Back/PROMO, etc.)
 *   - bracket-content → "<setCode> <number>" if it contains a space,
 *                       otherwise just <number>
 *   - paren-content → setLongName
 *
 * Returns:
 * {
 *   pokemonName: "Charizard VMAX HR" | "Pikachu" | "Radiant Charizard K",
 *   designation: "PROMO" | "SA" | "Old Back/PROMO" | null,
 *   setCode:     "S-P" | "S10b" | "PMCG-P" | "SM7b" | null,
 *   cardNumber:  "104" | "074/071" | "074/073" | "037/050" | "No.009",
 *   setLongName: 'S-P Promotional cards' | 'Sword & Shield "Champion\'s Path"',
 *   isPromo:     boolean,
 *   rawName:     <original>,
 * }
 *
 * Returns null only for non-string / empty input. Unparseable names
 * (no trailing `[...](...)` structure) return an object with most
 * fields null + the original rawName for caller fallback.
 */
export function parseSnkrdunkProductName(name) {
  ...
  // Generic regex: <leading> [optional: ":" <designation>] [<bracket>] (<paren>)
  // Notes:
  //   - leading is non-greedy so it stops at the first ":" or "[" it can hit
  //   - designation captures everything after ":" up to but not including "["
  //   - \s* between "]" and "(" tolerates the space-separated variant we saw
  //     on "Pikachu : PROMO[S-P 227] (Promotional Cards ...)"
  const m = raw.match(
    /^(.+?)\s*(?::\s*([^[]*?))?\s*\[([^\]]+)\]\s*\(([^)]+)\)\s*$/,
  );
  if (!m) {
  ...
  const designation = m[2] ? m[2].trim() : null;
  const bracketContent = m[3].trim();
  const setLongName = m[4].trim();

  // Split bracket on first whitespace: "<setCode> <number>" or just "<number>".
  // Pokemon bracket forms: "S-P 104", "S10b 074/071", "PMCG-P No.009",
  //                        "074/073" (no setCode), "025/214" (no setCode),
  //                        "SM7b 037/050".
  const bracketSplit = bracketContent.match(/^(\S+)\s+(.+)$/);
  let setCode = bracketSplit ? bracketSplit[1] : null;
  let cardNumber = bracketSplit ? bracketSplit[2] : bracketContent;
  if (!bracketSplit) {
    // Single-token bracket: decide whether it's a number or a setCode.
    // Number-ish tokens start with a digit or a "No." prefix ("074/073",
    // "025/214", "No.009", "104a"). Everything else ("PMCG3", "PMCG-P",
    // "e4") is a setCode-only bracket — vintage products often carry the
    // set without a card number. Pre-2026-06 this path misclassified
    // setCode-only brackets as cardNumber, leaving setCode null, which
    // (a) broke era scoring for ~3.7k stored candidates and (b) fed a
    // letters-only "number" into the number comparator.
    if (!/^(?:no\.?\s*)?\d/i.test(bracketContent)) {
  ...
  const isPromo =
    /promo/i.test(designation ?? "") || /promo/i.test(setLongName);
