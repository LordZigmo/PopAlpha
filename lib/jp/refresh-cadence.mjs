 ...
 *
 * yahoo_jp.hotSnkrdunkCovered: a hot card whose CANONICAL RAW Snkrdunk price
 * row was observed within JP_SNK_OWNERSHIP_FRESHNESS_HOURS gets its daily RAW
 * series from Snkrdunk; Yahoo re-checks at 96h instead of double-scraping the
 * same card daily on both sources. Ownership is demonstrated by freshness,
 * not row existence: graded-/per-printing-only presence, code-null rows,
 * suppression-parked products, and starved rows all age out of the window and
 * hand the daily cadence back to Yahoo automatically.
 *
 ...
 */
/**
 * How recent a canonical RAW Snkrdunk observation must be for Snkrdunk to
 * count as owning a hot card's daily series (the scan_yahoo_refresh_candidates
 * EXISTS window). Must comfortably exceed the snkrdunk hot cadence or healthy
 * ownership would flap; smoke-asserted >= 2x snkrdunk.hot.
 */
export const JP_SNK_OWNERSHIP_FRESHNESS_HOURS = 96;
