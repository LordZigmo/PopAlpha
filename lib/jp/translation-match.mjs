...
// Variant suffixes that don't affect species identity. Stripped before
// the STRICT_MATCH comparison so "Charizard ex" matches "Charizard ex"
// across capitalization, "Pikachu V" matches "Pikachu V", etc.
const VARIANT_SUFFIX_RE = /\s+(ex|gx|v|vmax|vstar|lv\s*\.x)$/i;
 ...
 * strip a single trailing variant suffix. Exported because the unit
 * tests want to assert the normalization shape directly.
 */
...
export function pickPairings(enCard, candidates, opts = {}) {
  ...
  for (const cand of scored) {
    ...
    if (accepted.length > altRankMax + 1) break;
  }
