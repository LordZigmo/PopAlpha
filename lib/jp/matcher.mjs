...
export function extractGrade(listing) {
  ...
  if (m) {
    ...
    if (grade >= 1 && grade <= 10) {
      ...
      // Only emit labels for tiers that have an exact card_metrics
      // bucket. Half-grades like BGS 8.5 / CGC 7.5 / BGS 6.5 don't
      // have a card_metrics target — Codex P2 on PR #58 caught that
      // dumping them into LE_7 would skew the joined median into a
      // wrong bucket once graded ingestion turns on (e.g., BGS 8.5
      // sales recorded as "7 or less"). They fall through to
      // GRADED_UNKNOWN so the orchestrator skips them, same as the
      // 鑑定品-without-grade case below.
      let label;
      if (grade === 10) label = "G10";
      else if (grade === 9.5) label = "G9_5";
      else if (grade === 9) label = "G9";
      else if (grade === 8) label = "G8";
      else if (Number.isInteger(grade) && grade <= 7) label = "LE_7";
      else label = "GRADED_UNKNOWN"; // half-grades 1.5–8.5 — no bucket
      return {
