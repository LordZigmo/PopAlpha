...
export function scoreListing(listing, canonicalCard, opts = {}) {
  ...
  const baseNumber = baseCardNumber(cardNumber);
  if (baseNumber && titleMatchesCardNumber(title, baseNumber)) {
    score += 0.15;
...
// =============================================================================
// 2b. Card-number containment + mismatch filter
// =============================================================================
// Number-extraction tolerance shared by the scorer bonus and the mismatch
// filter: ASCII digits / ASCII slash only, with non-digit boundaries.
// Scraped titles are stored as-is (scripts/scrape-yahoo-jp.mjs trims but
// does NOT normalize), so full-width digits/slashes (２４６／１９３) are
// invisible to both — such listings neither earn the +0.15 bonus nor get
// filtered.

/**
 * Base card number for title matching: canonical_cards.card_number minus
 * any "-suffix" disambiguator we synthesized at import (the listing title
 * won't have it). Returns null when the card has no usable number.
 */
export function baseCardNumber(cardNumber) {
...
/**
 * Containment test behind scoreListing's +0.15 card-number bonus (same
 * regex that lived inline there). Boundaries: the number must not sit
 * inside a longer digit run; a trailing "/<set-total>" is allowed so
 * "126" matches "126/193".
 */
export function titleMatchesCardNumber(title, baseNumber) {
  const numberRegex = new RegExp(`(?:^|[^0-9])${baseNumber}(?:[^0-9]|$|/\\d+)`);
  return numberRegex.test(String(title ?? ""));
}

// "number/set-total" fractions in listing titles ("126/193"). Boundary
// guards extend the scorer's digit-run rule — a negative signal must be
// stricter than a +0.15 bonus:
//   • numerator not preceded by a digit or "/" (date chains like
//     "2026/05/10" and embedded digit runs never yield phantom numbers)
//   • denominator >= 40: real JP set totals on single-card listings run
//     >= ~48, while date fractions max out at /31 and condition scores
//     ("10/10") are smaller still. Tiny-total sets (e.g. /025 promo
//     packs) simply never parse → never filtered.
const TITLE_NUMBER_FRACTION_RE = /(?:^|[^0-9/])(\d{1,3})\/(\d{2,3})(?![0-9])/g;
const MIN_PLAUSIBLE_SET_TOTAL = 40;

/** All parseable card-number fractions in a title → [{ numerator, denominator }]. */
export function extractTitleNumberFractions(title) {
  const out = [];
  for (const m of String(title ?? "").matchAll(TITLE_NUMBER_FRACTION_RE)) {
    const numerator = Number.parseInt(m[1], 10);
    const denominator = Number.parseInt(m[2], 10);
    if (denominator >= MIN_PLAUSIBLE_SET_TOTAL) out.push({ numerator, denominator });
  }
  return out;
}

/**
 * Negative number-mismatch filter for the fan-out contamination class.
 *
 * buildPrecisionQuery searches name+set with no card number, so every
 * same-name slug in a set pools the SAME sold listings — MEGA Dream ex
 * "Mega Dragonite ex" exists at #126 (RR, ~$17) and #246 (SAR, ~$187),
 * and both slugs carried one identical blended median (SAR money
 * inflating the regular print, RR bulk deflating the SAR). When the
 * canonical card's number is known and a listing's title carries a
 * parseable number that does NOT match it, drop that listing for THIS
 * card only. Deliberately conservative: listings with no parseable
 * number stay pooled, cards with no/non-numeric card_number filter
 * nothing, and anything the scorer's number bonus would credit is always
 * kept — a smaller honest sample beats a blended dishonest one.
 */
export function shouldExcludeForNumberMismatch(title, cardNumber) {
  const base = baseCardNumber(cardNumber);
  if (!base) return false;
  const leadingDigits = base.match(/^\d{1,4}/);
  if (!leadingDigits) return false;
  if (titleMatchesCardNumber(title, base)) return false;
  const fractions = extractTitleNumberFractions(title);
  if (fractions.length === 0) return false;
  const want = Number.parseInt(leadingDigits[0], 10);
  return !fractions.some((f) => f.numerator === want);
}
...
export function selectMatched(listings, canonicalCard, opts = {}) {
  ...
  // Number-mismatch filter sits at the aggregation boundary so the
  // per-printing splits AND the canonical rollup below both inherit it
  // (the rollup is where the same-name fan-out contamination bites).
  let numberMismatchExcluded = 0;
  const accepted = scored.filter((s) => {
    if (s.score < minScore) return false;
    if (shouldExcludeForNumberMismatch(s.listing.title, canonicalCard.card_number)) {
      numberMismatchExcluded += 1;
      return false;
    }
    return true;
  });
  ...
  if (accepted.length === 0 && filtered.length > 0) {
    warnings.push(
      numberMismatchExcluded > 0
        ? `all candidate listings dropped (score < minScore, or card-number mismatch ×${numberMismatchExcluded})`
        : "all listings scored below minScore — query may be matching wrong card",
    );
  }
