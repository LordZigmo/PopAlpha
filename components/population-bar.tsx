...
export default function PopulationBar({ higherShare, topTierShare }: { higherShare: number | null; topTierShare: number | null }) {
  ...
  const notHigher = topTierShare ?? 0;
  ...
  return (
    <div className="glass density-card rounded-[var(--radius-card)] border-app border p-[var(--space-card)]">
      ...
      <div className="mt-3 flex h-3 overflow-hidden rounded-full border-app border bg-surface">
        ...
        <div style={{ width: `${notHigher * 100}%`, background: "var(--color-positive)" }} />
      </div>
      <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
        <p className="text-muted">Higher: <span className="text-negative tabular-nums">{pct(higherShare)}</span></p>
        <p className="text-muted">Not-higher: <span className="text-positive tabular-nums">{pct(topTierShare)}</span></p>
      </div>
    </div>
  );
}
