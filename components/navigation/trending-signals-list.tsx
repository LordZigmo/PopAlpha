...
export default function TrendingSignalsList() {
  ...
  const signalCards = (
    <div className="mt-3 space-y-2.5">
      ...
      >
        <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#6B7280]">1. Bullish leader</p>
        ...
        <p className="mt-1 text-[11px] text-[#8DF0B4]">{bullishLeader.up_pct}% of the board expects upside across {bullishLeader.vote_count} votes</p>
      </Link>
      ...
      <Link
        ...
        className="block rounded-[0.95rem] border border-white/[0.03] bg-[#0B0B0B] px-3 py-3 transition hover:border-white/[0.08]"
      >
        <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#6B7280]">2. Most watched</p>
        ...
        <p className="mt-1 text-[11px] text-[#D4D4D8]">{mostWatched.add_count} collectors added it to a watchlist in the last hour</p>
      </Link>
      ...
      <Link
        ...
        className="block rounded-[0.95rem] border border-white/[0.03] bg-[#0B0B0B] px-3 py-3 transition hover:border-white/[0.08]"
      >
        <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#6B7280]">3. Attention gap</p>
        <p className="mt-1 text-[12px] font-semibold text-white">{divergence.name}</p>
        <p className="mt-1 text-[11px] text-[#8A8A8A]">{divergence.set_name ?? "Views outrunning conviction"}</p>
        <p className="mt-1 text-[11px] text-[#D4D4D8]">{divergence.total_views} views, but only {divergence.vote_count} collectors priced a move</p>
      </Link>
    </div>
  );
  ...
  return (
    <div className="relative mt-4 overflow-hidden rounded-[1.35rem] border border-[#1E1E1E] bg-[#101010] px-4 py-4">
      <div className="flex items-center justify-between gap-3">
        <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#6B6B6B]">Signal board</p>
        ...
        <div className="pointer-events-none absolute inset-x-4 top-1/2 z-10 flex -translate-y-1/2 justify-center">
          <div className="pointer-events-auto flex flex-col items-center gap-3 rounded-[1.2rem] border border-white/10 bg-[#090909]/88 px-5 py-5 text-center shadow-[0_18px_45px_rgba(0,0,0,0.35)] backdrop-blur-md">
            <p className="max-w-[13rem] text-[12px] font-medium leading-5 text-[#CFCFCF]">
              Create a free account to unlock the live signal board and save the names worth revisiting.
            </p>
            <Link
              href="/sign-up"
