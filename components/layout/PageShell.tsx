...
import GenericRightRail from "@/components/layout/GenericRightRail";

/**
 * Reusable top-level page shell for browsing pages — identical layout chrome
 * to {@link CanonicalCardShell}: fixed header, `max-w-[1600px]`, and a
 * 70/30 desktop grid (main content left, sticky right rail right). Mobile
 * drops to a single column with the bottom nav.
 *
 * Pass `backHref` for back-button pages; omit on top-level pages and the
 * logo becomes the sole left-side anchor.
 *
 * Pass `rightRail` for a page-specific rail; omit and the {@link GenericRightRail}
 * (nav shortcuts) is used site-wide.
 */
export default function PageShell({
  ...
  const resolvedRightRail = rightRail ?? <GenericRightRail />;
