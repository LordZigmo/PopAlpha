...
**The release candidate is now `77570f7`** (was `bba6f9d`). Build the TestFlight archive from current `main` HEAD (`77570f7`), build number **`2026.5.28`** (marketing version `1.0.0`). Deltas since `bba6f9d`: PR #128 (iOS price copy), #129 (doc sync), #130 (build-number bump — `2026.5.24` was already uploaded as the May 24 build, so App Store Connect would reject a re-upload at that number), #131 (P0–P2 robustness hardening).
...
- **Still outstanding (not trust-blocking):** signed TestFlight archive from `77570f7` → App Store Connect. iOS on-device display parity for the new states (state badges, recent-market-signal context, the Market Watch rail) remains deferred — cosmetic and decode-safe, not price-correctness.
