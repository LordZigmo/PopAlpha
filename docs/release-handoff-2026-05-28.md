...
- **Daily PriceCharting ingestion automated (PR #139).** Root cause of the "only ~139 cards priced within 24h" symptom: the trusted-price rails (`canonical_trusted_raw_prices`) are populated only by the PriceCharting feed, which had been run by hand (last observation `2026-05-27`) and then stopped — so the trustworthy-price view fails closed on stale data even though base `card_metrics` is healthy. Fix: a scheduled cron `/api/cron/import-pricecharting` (daily `0 11 * * *` UTC) now runs the same pipeline the manual CLI did. The CLI orchestration was extracted into `lib/backfill/pricecharting-ingest.ts` so the cron and `scripts/import-pricecharting-products.mjs` share one code path. **Mon–Sat** ticks are lean (upsert products + observations + trusted-price parity RPC) to keep the ~14.8k already-matched cards fresh inside the 300s function ceiling; **Sunday (UTC)** ticks also re-match the full EN catalog to pick up newly released products. `?match=1` / `?match=0` override per-tick for ops.
  - **✅ Configured + verified (2026-05-29).** `PRICECHARTING_CSV_URL` is set in Vercel (Production + Preview) and the feed was triggered manually to confirm: 87.8k CSV records → 85,385 products + 75,554 observations upserted, parity RPC refreshed 55,451 trusted rows, run took **86s**. **Priced canonical cards jumped ~139 → ~12,925** (10,587 ALIGNED + 2,097 PUBLIC_ONLY + 241 SIGNAL_HIGHER/LOWER; ~108k remain NO_RELIABLE_PRICE/UNDER_REVIEW by design). One gotcha hit during setup: the env var was first created as `RICECHARTING_CSV_URL` (missing the leading `P`) — the route reads `process.env.PRICECHARTING_CSV_URL`, so the exact key matters.
- **Trusted-price freshness alarm added (PR #141).** `/api/cron/check-pricecharting-freshness` (daily `0 14 * * *` UTC) returns HTTP 500 — surfaced as a failed Vercel cron — if `canonical_trusted_raw_prices` hasn't been refreshed in > 26h, i.e. the import silently stopped (expired CSV URL, cron error). Mirrors `check-fx-rates-health`. Keep Vercel cron-failure notifications enabled to receive it. Closes the "feed stops unnoticed → priced count collapses to ~139" failure mode that prompted PR #139.
- **Still outstanding:** (1) signed TestFlight archive from current `main` HEAD → App Store Connect; (2) on-device, confirm the price parity (Claydol/Cobalion detail == homepage) and a populated Market Watch tab. (Optional hygiene: unify `CRON_SECRET` across Vercel Prod + Preview + the GitHub Actions secret — prod and preview currently differ, so the manual `psa-ingest-cron` workflow 401s; not launch-critical.)
