...
| Phase | Status | Notes |
| --- | --- | --- |
| 1. Snapshot pipeline (PSA official API) | `shipped` (merged #221; apply migration + smoke-test cron) | `psa_spec_targets` rotation + `psa_spec_pop_snapshots` daily rows via `GET /publicapi/pop/GetPSASpecPopulation/{specID}`; cron `snapshot-psa-pop` 07:50 UTC, 60-call budget (PSA free tier ~100/day shared with cert lookups). SpecIDs harvested from every slab scan + seeded from historic cert lookups. |
| 2. SpecID → catalog mapping | `in progress` (engine built; owner verification + set-map curation pending) | Handoff + decisions: `docs/psa-specid-mapping-handoff.md`. `psa_set_map` (brand backbone, curated>derived) + `psa_spec_card_map` (slug+printing, confidence, queue reasons, verified flag); match-on-arrival in `/api/psa/cert`, daily sweep cron `match-psa-specs` 08:20 UTC (DB-only, zero PSA quota), `?mode=report` = audit artifact. |
| 2b. SpecID inventory expansion (whole catalog) | `in progress` (engine built; GetSetItems mechanics unverified — owner runs script first) | Scans only harvest specs we've seen (4 to date); whole-catalog coverage scrapes PSA pop-report set pages (`POST /Pop/GetSetItems`): one fetch enumerates every spec in a set AND its grade distribution → `psa_spec_targets` (source `pop_scrape`) + same-day `psa_spec_pop_snapshots` (source `pop_scrape`), zero official-API quota. `psa_pop_set_pages` registry (owner-seeded; pages carry `canonical_set_code` so discovered specs match with set identity known). Cron `discover-psa-specs` built but UNSCHEDULED until Cloudflare-vs-Vercel egress verified; `scripts/discover-psa-specs.mjs` is the residential-IP path. Matched specs get priority floor 10 so the 60-call API budget favors mappable specs. |
