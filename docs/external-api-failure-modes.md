...
3. ~~**Add a smoke check to CI/preview** that hits each AI cron with `?maxCards=1` and asserts `llmGenerated > 0`.~~ — **DONE (2026-05-28).** `scripts/check-ai-cron-smoke.mjs` (npm `check:ai-cron-smoke`) hits `refresh-card-profiles` (`?mode=refresh&maxCards=1`) and `refresh-ai-brief`, asserting the LLM actually produced output (`llmGenerated > 0`; a brief with `source="llm"` + output tokens) and FAILING on `llmPathDegraded` / `ok:false`. A `totalProcessed=0` / no-data run is a non-fatal WARN (couldn't exercise the model), not a false red. Needs a server with live AI creds + `CRON_SECRET`, so it runs against a preview/local URL (`SMOKE_BASE_URL`) — NOT in the static build; it SKIPS (exit 0) when those env vars are unset. Cost ~$0.001/run. Would have caught Incident 2 within the deploy that broke it. **CI:** `.github/workflows/ai-cron-smoke.yml` runs it automatically on each successful Vercel preview deploy (`deployment_status` → preview `environment_url`); requires a `CRON_SECRET` Actions secret + live AI creds in the Preview env. Preview deploys sit behind Vercel Deployment Protection, so it also sends `x-vercel-protection-bypass` from `VERCEL_AUTOMATION_BYPASS_SECRET` — if that secret is missing/stale the request hits Vercel's edge auth wall (HTML 401, `_vercel_sso_nonce`) before reaching the route. The check treats that protection-wall 401 as a loud non-fatal **WARN** ("LLM path NOT validated — set the bypass secret"), distinct from a route-level `requireCron` 401 (JSON), which stays a hard FAIL — so a stale bypass secret doesn't masquerade as LLM degradation or false-red the build.
