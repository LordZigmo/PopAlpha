...
3. ~~**Add a smoke check to CI/preview** that hits each AI cron with `?maxCards=1` and asserts `llmGenerated > 0`.~~ — **DONE (2026-05-28).** `scripts/check-ai-cron-smoke.mjs` (npm `check:ai-cron-smoke`) hits `refresh-card-profiles` (`?mode=refresh&maxCards=1`) and `refresh-ai-brief`. **Narrowed 2026-05-29:** it HARD-FAILS only on the Incident-2 silent lie — a successful-looking `HTTP 200 + ok:true` response where the LLM nonetheless never ran (`llmPathDegraded`, or `totalProcessed>0` with `llmGenerated=0`, or no `source="llm"` brief). Everything else is a non-fatal **WARN** (exit 0): non-2xx, `ok:false`, 401, transport/timeout, or no-data. Rationale: those are LOUD failures (already visible) or "couldn't validate here" (e.g. a Preview env with no AI creds) — not the silent fallback this guards, so they must not false-red the build. A creds-less env therefore warns (honest no-op); point it at an env with AI creds for a hard signal. Needs a server with live AI creds + `CRON_SECRET`, so it runs against a preview/local URL (`SMOKE_BASE_URL`) — NOT in the static build; it SKIPS (exit 0) when those env vars are unset. Cost ~$0.001/run. Would have caught Incident 2 within the deploy that broke it. **CI:** `.github/workflows/ai-cron-smoke.yml` runs it automatically on each successful Vercel preview deploy (`deployment_status` → preview `environment_url`); requires a `CRON_SECRET` Actions secret + live AI creds in the Preview env. Preview deploys sit behind Vercel Deployment Protection, so it also sends `x-vercel-protection-bypass` from `VERCEL_AUTOMATION_BYPASS_SECRET` — if that secret is missing/stale the request hits Vercel's edge auth wall (HTML 401, `_vercel_sso_nonce`) before reaching the route. Both that protection-wall 401 and a route-level `requireCron` 401 (JSON, CRON_SECRET mismatch) are WARNs — in either case the LLM path was never reached, so neither is the silent fallback this guards. (History: the strict version false-red the build when the Preview env legitimately couldn't run the LLM — first a stale `VERCEL_AUTOMATION_BYPASS_SECRET`, then a Preview `CRON_SECRET` mismatch, then missing Preview AI creds — which is what motivated the 2026-05-29 narrowing above.)
4. ~~**Generalize the "100 % degradation = ok:false" rule into a small shared helper.**~~ — **DONE (2026-05-28).** `lib/ai/llm-degradation.ts` exports `isLlmPathDegraded({ processed, llmCount })`; `refresh-card-profiles` uses it. (`refresh-ai-brief` keeps its item-level `degradedBriefs.length > 0` signal — a distinct shape intentionally not folded into the count-based helper.)
