...
**EN-RAW moderate diverged floor (`20260702050000`):** extends the cheap floor above the `$2` band. A `PRICECHARTING_DIVERGED` EN-RAW row with `greatest(scrydex, pricecharting) > 2` whose stored `relative_delta_pct` (the same `|Δ| / midpoint × 100` metric the classifier gates MATCH on at 35) is `<= 60` surfaces `least(scrydex, pricecharting)` instead of hard-nulling. Motivating case (user report 2026-07-01): `chaos-rising-104-mega-dragalge-ex`, Scrydex `$4.42` vs PriceCharting `$6.32` = 35.4% delta — a hair over MATCH on a freshly released set still in price discovery; blank hero for weeks otherwise (1,689 rows in the band at rollout; 1,744 wilder disagreements stay suppressed). Surfaced rows: confidence **20** (below the cheap floor's 25 — real dollars, active disagreement), `market_low_confidence = true`, NO change badge, `latest_price` pinned to the floor, `market_blend_policy = 'POPALPHA_MARKET_MODERATE_DIVERGED_FLOOR'`, `market_price_display_state = 'PRICECHARTING_MODERATE_DIVERGED'`, provenance `confidenceStatus = LOW` / `publicInputStatus = SUPPORTED` / no `quarantineReason` / `internalGuardrailStatus = DIVERGED` retained. `relative_delta_pct` null → stays suppressed (stale classifier rows never sneak through). `> 60%` (the Latias `$564`-on-`$5` class) stays hard-suppressed with the iOS diverged note. Integrity check: an EN-RAW row with the MODERATE policy must have non-null `market_price > $2` equal to `least(scrydex, pricecharting)`, confidence 20, and null `change_pct_24h/7d`.
