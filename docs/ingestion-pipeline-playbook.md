...
- **Don't let the rollup drain's `refresh_price_changes_for_cards` touch JP-language slugs.** `change_pct_24h/7d` for `canonical_cards.language = 'JP'` rows is owned exclusively by `compute_jp_card_price_changes()`; when `refresh_price_changes_core()`'s scope leaked onto JP rows, the twice-hourly drain clobbered the 12-hourly JP-native deltas ~500:1 (fixed PR #230). Full incident + writer-partition rule: `docs/derived-metric-integrity-playbook.md` (2026-06-11 entry).
