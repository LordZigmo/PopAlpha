-- supersedes: 20260501010100_compute_daily_top_movers_outlier_cap.sql
--
-- Add a recency multiplier to the homepage rails composite score.
--
-- Diagnosis (2026-05-04): rails feel flat because ~45% of slots come back
-- pre-2010 vintage. The composite score
--
--     abs(change_pct) × (confidence/100) × liquidity_multiplier
--
-- has no notion of "is this a card people are excited about right now."
-- Vintage's structural volatility (thin markets → big swings on a single
-- sale) wins against modern cards' moderate moves on equal footing.
-- A typical visitor cracking Prismatic Evolutions sees Neo Revelations
-- Suicune as the top mover.
--
-- Fix: multiply by a release-year boost so current-year sets get
-- ~2.5× the ranking weight, recent past gets ~1.5×, vintage stays at
-- 1.0× (no penalty — they still surface when their moves are dramatic
-- enough to overcome the relative gap). This shifts the *ordering* of
-- candidates without changing the filter (eligibility unchanged): the
-- 75% outlier cap, $20 premium minimum, snapshot_count_30d ≥ 27,
-- confidence ≥ 45, and max_per_set = 2 all preserved verbatim.
--
-- Calibration (year is from canonical_cards.year, NULL → 1.0 via fall-
-- through):
--   ≥ 2025 — current/last year, peak buzz       → 2.5×
--   ≥ 2023 — recent in-print era (151/CZ/etc.)  → 1.5×
--   ≥ 2020 — late SwSh / early SV               → 1.2×
--   else  — vintage / classic                  → 1.0×
--
-- All other behaviour preserved verbatim from 20260501010100. Body
-- pulled directly via pg_get_functiondef so no other clauses drift.
--
-- Rollback: re-apply 20260501010100's body (drop the recency case).
  ...
  with candidates as (
    select
      ...
      abs(coalesce(pcm.change_pct_24h, pcm.change_pct_7d)) *
        ...
        -- Recency multiplier (NEW in 20260504220000).
        case
