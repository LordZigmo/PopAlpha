...
-- 1. Public RAW current prices are Scrydex-only and freshness-gated.
-- 2. Public RAW sample depth / liquidity counts exclude stale JustTCG history.
-- 3. Public RAW confidence / provenance stop claiming JustTCG live support.
-- 4. Public RAW price changes require a live current Scrydex price.
...
begin
  ...
  provider_counts as (
    select
      ph.canonical_slug,
      0::numeric as justtcg_points_7d,
  ...
  normalized as (
    select
      ...
      0::numeric as source_mix_justtcg,
      case
        when w.w_scrydex > 0 then 1::numeric
        else 0::numeric
      end as source_mix_scrydex,
      null::text as justtcg_excluded_reason,
      case
