...
alter table public.card_metrics
  add column if not exists scrydex_price numeric null;

update public.card_metrics
set scrydex_price = coalesce(scrydex_price, pokemontcg_price)
where scrydex_price is null
  and pokemontcg_price is not null;
