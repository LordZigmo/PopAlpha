-- supersedes: 20260528014618_homepage_movers_require_public_price_policy.sql
--
...
--
-- Follow-up. The old hot/warm freshness coverage gate counted any public
-- market_price row. Once PriceCharting became private-only, many rows
-- correctly expose market_price = null, so that gate started blocking
-- recomputes even when a healthy public-policy-eligible mover pool
-- existed. The gate now measures the same public-policy-eligible 24h
-- pool that can actually enter daily_top_movers.
--
-- Reversal: re-apply 20260528014618_homepage_movers_require_public_price_policy.sql.
-- No data migration.
...
declare
  ...
  _public_eligible_fresh_24h int;
  _public_eligible_threshold int := 25;
begin
  ...
  select count(*) into _public_eligible_fresh_24h
  from public.public_card_metrics pcm
  join public.canonical_cards cc on cc.slug = pcm.canonical_slug
  where pcm.grade = 'RAW'
    and pcm.printing_id is null
  ...
  if _public_eligible_fresh_24h < _public_eligible_threshold then
    return jsonb_build_object(
      'computed', false,
      'reason', 'public_policy_coverage_too_low',
      'public_eligible_fresh_24h', _public_eligible_fresh_24h,
      'public_eligible_threshold', _public_eligible_threshold,
  ...
  return jsonb_build_object(
    ...
    'coverage_gate', '20260528014845_public_policy_eligible_24h',
