-- 20260303120000_card_metrics_dual_scope_rollup.sql
--
-- Writes card_metrics at two scopes from the same refresh:
--   1) printing-scoped rows (printing_id populated)
--   2) canonical aggregate rows (printing_id NULL)
--
-- Several app surfaces (set pages, search, aggregate card reads) depend on
-- the canonical row. Without it, data can exist at the variant layer while
-- appearing "missing" in the app.
...
begin
  ...
  all_prices as (
    ...
    -- Duplicate printing-scoped points into a canonical aggregate bucket so
    -- frontend reads that rely on printing_id IS NULL always have a stable row.
    select
      canonical_slug,
