-- 20260605040000_set_caps_raw_only.sql
--
-- supersedes: 20260604200000_set_summary_change_aligned_baseline.sql
-- supersedes: 20260604210000_set_finish_change_aligned_baseline.sql
--
-- Exclude GRADED prices from set market caps + change basis. The set functions
-- summed latest_price over ALL of variant_price_latest, including graded
-- variant_refs (stamped grade='RAW' but identifiable by the variant_ref segment),
-- so set "market caps" were ~97% graded: e.g. set 151's all-variants cap was
-- $206,326 vs $2,956 of actual raw cards (base / Gym Challenge / Prismatic all
-- ~97% graded). This filters the source CTEs to RAW variant_refs only -- no
-- '::GRADED::' segment and no '::PSA|CGC|BGS|TAG::' grader segment (covers both
-- the long `<pid>::<set-pt>::GRADED::PROV::bucket::RAW` form and the short
-- `<pid>::PROV::bucket` form) -- so market_cap / market_cap_all_variants / the
-- per-finish caps + the change basis are raw-only.
--
-- The displayed headline cap (set_summary.market_cap, the primary one-variant-
-- per-card value) was already mostly raw; this fixes the secondary per-finish
-- "tracked value" line and the all-variants total, and guarantees the primary
-- selection can no longer pick a graded variant. Pinned check-set-summary-view-
-- contract fragments are untouched -- the filter lives only in WHERE clauses,
-- not in any SELECT output expression.
--
-- Bodies are the live definitions (20260604200000 / 20260604210000) reproduced
-- verbatim EXCEPT the added raw-only WHERE predicates.

CREATE OR REPLACE FUNCTION public.refresh_set_finish_summary_latest(only_set_ids text[] DEFAULT NULL::text[])
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
declare
...
begin
  ...
  with filtered as (
    ...
    where set_id is not null
      ...
      -- Raw-only: exclude graded variant_refs (both long ::GRADED:: and short
      -- ::PROV:: forms) so set caps + change are not inflated by graded prices.
      and variant_ref !~ '::GRADED::'
      and variant_ref !~ '::(PSA|CGC|BGS|TAG)::'
...
CREATE OR REPLACE FUNCTION public.refresh_set_summary_snapshots(target_as_of_date date DEFAULT CURRENT_DATE, only_set_ids text[] DEFAULT NULL::text[])
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
declare
  ...
  with effective_prices as (
    ...
    from (
      ...
      where vpd0.as_of_date = target_as_of_date
        ...
        -- Raw-only: exclude graded variant_refs from set caps + change.
        and vpd0.variant_ref !~ '::GRADED::'
        and vpd0.variant_ref !~ '::(PSA|CGC|BGS|TAG)::'
      ...
      where target_as_of_date = current_date
        ...
        -- Raw-only: exclude graded variant_refs from set caps + change.
        and vpl.variant_ref !~ '::GRADED::'
        and vpl.variant_ref !~ '::(PSA|CGC|BGS|TAG)::'
