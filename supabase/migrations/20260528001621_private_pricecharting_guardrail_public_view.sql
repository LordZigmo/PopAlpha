...
-- it to veto/validate permitted inputs, but must not expose PriceCharting as a
-- displayed source or directly publish its raw price.
  ...
  select
      ...
      when j.is_en_raw then
        case
          when j.private_trust_status = 'PRICECHARTING_SCRYDEX_MATCH'
            then j.private_scrydex_price_usd
    ...
    case
      when j.is_en_raw then
        case
          when j.private_trust_status = 'PRICECHARTING_SCRYDEX_MATCH'
            then coalesce(j.private_scrydex_as_of, j.market_price_as_of)
          when j.private_trust_status in ('PRICECHARTING_PRIMARY', 'PRICECHARTING_DIVERGED', 'NO_TRUSTED_PRICE')
            then null
          when j.raw_market_price_outlier
            then null
...
select
  ...
  justtcg_price,
  case when raw_market_price_outlier then null else scrydex_price end as scrydex_price,
  case when raw_market_price_outlier then null else scrydex_price end as pokemontcg_price,
