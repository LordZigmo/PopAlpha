-- Graded freshest hero + 14-day median — surface them via public_card_metrics.
--
-- Price-display redesign, graded step. The graded card_metrics rollup
-- (lib/backfill/graded-variant-metrics-writer.ts) now also writes latest_price
-- (freshest sold point) + display_price (14-day median), computed from the SAME
-- per-(slug,printing,grade) points it already medians for median_30d — so the
-- hero/median can never disagree with the graded analytics on the row.
--
-- This migration only re-points the view's graded path at those columns:
--   * market_price  -> display_price (14-day median)        [the median sub-line]
--   * latest_price  -> latest_price  (freshest sold point)  [the hero]
-- for graded rows (grade <> 'RAW'). It is PURELY ADDITIVE: graded market_price /
-- latest_price are NULL for all 189,965 graded rows today (verified), so the new
-- coalesce branches only ever turn a NULL into a value, never change an existing
-- one. EN-RAW and JP-RAW paths are untouched; median_7d / median_30d stay exposed
-- as before (iOS falls back to the 30d median when nothing landed in 14 days).
--
-- supersedes: 20260602040000_jp_freshest_and_14d_median.sql
--             (public_card_metrics — latest prior body. Diffed: identical except
--              four added graded CASE branches in public_market_price /
--              _as_of / public_latest_price / _as_of. No column added, dropped,
--              reordered or repurposed; the jp_* trailing columns are preserved.)
