-- Make canonical RAW market price Scrydex-first and compute change windows
-- from a single provider stream so all card surfaces inherit the same policy.
...
begin
  ...
  scored as (
    select
      ...
      case
        when n.scrydex_price is not null and n.scrydex_excluded_reason is null then coalesce(n.scrydex_as_of, n.current_market_price_as_of)
        when n.justtcg_price is not null and n.justtcg_excluded_reason is null then coalesce(n.justtcg_as_of, n.current_market_price_as_of)
        when n.scrydex_price is not null then coalesce(n.scrydex_as_of, n.current_market_price_as_of)
        when n.justtcg_price is not null then coalesce(n.justtcg_as_of, n.current_market_price_as_of)
...
as $$
begin
  return public.refresh_card_market_confidence_core(null);
end;
$$;
...
begin
  ...
  return public.refresh_card_market_confidence_core(v_scope);
end;
$$;
