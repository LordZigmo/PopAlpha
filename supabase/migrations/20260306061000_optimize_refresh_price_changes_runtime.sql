-- Reduce refresh_price_changes runtime to avoid PostgREST upstream timeouts.
-- Strategy:
-- 1) Restrict to canonical RAW slugs that actually exist in card_metrics.
-- 2) Prefer snapshot points; only fall back to 30d points when snapshot is absent.
-- 3) Add an index aligned with provider/source_window/ts filtering.

create index if not exists price_history_points_provider_window_ts_slug_idx
  on public.price_history_points (provider, source_window, ts desc, canonical_slug);
...
begin
  ...
  with slugs_with_history as (
    select distinct rp.canonical_slug
    from recent_points rp
  ),
  do_null as (
    update public.card_metrics cm
