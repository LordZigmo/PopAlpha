-- Extend market_snapshot_rollups to include JUSTTCG observations alongside
-- EBAY and TCGPLAYER so that JustTCG Near Mint prices (written by the
-- sync-justtcg-prices cron) contribute to median calculations and every
-- matched card shows a price instead of "Collecting".
...
with base as (
  ...
  where source in ('EBAY', 'TCGPLAYER', 'JUSTTCG')
    and currency = 'USD'
    and price_value > 0
