-- Blend PopAlpha's PriceCharting-derived 24h/7d change into public_card_metrics
-- for EN-RAW *price-corroborated* cards, so the movers rails + change displays
-- light up (~7,000+ cards) without lowering the trust bar.
--
-- Problem: a confident EN-RAW change only surfaces when the Scrydex-derived
-- change (change_pct_24h / change_pct_7d) is present, but the Scrydex daily-
-- snapshot history is currently sparse — so ~117 EN-RAW cards show a change
-- today even though ~7k are price-corroborated (private_trust_status =
-- 'PRICECHARTING_SCRYDEX_MATCH'). Both the Scrydex change and the PriceCharting
-- change are PopAlpha-computed metrics; the trust comes from the two-source
-- price corroboration, not from which series the % was computed off.
--
-- Fix: inside the EXISTING MATCH-only trust gate, COALESCE the Scrydex change to
-- PopAlpha's PriceCharting-derived change as a fallback when Scrydex's is absent,
-- outlier-capped at |%| <= 200 (matching the Scrydex path). The gate itself is
-- unchanged: non-MATCH EN-RAW still hard-nulls (conservative), and the headline
-- price / confidence / provenance columns are untouched. Two raw passthrough
-- columns (private_pricecharting_change_pct_24h / _7d, from
-- canonical_trusted_raw_prices) are added to joined_rows to feed the fallback.
--
-- supersedes: 20260602070000_jp_market_price_from_jp_series.sql
--             (public_card_metrics — identical body except two added joined_rows
--              columns + a PriceCharting fallback inside the MATCH branch of
--              public_change_pct_24h / public_change_pct_7d. No column added,
--              dropped, reordered or repurposed in the final SELECT.)
