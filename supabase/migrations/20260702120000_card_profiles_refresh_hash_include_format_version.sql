-- Include the profile FORMAT version in the refresh selector's current_hash so
-- it stays byte-identical to lib/ai/card-profile-fallback.ts buildMetricsHash.
--
-- Context: the summary generators now emit a {price} sentinel (filled with the
-- live Market Price at read time) instead of a baked dollar figure, so a cached
-- summary can't drift from the card-detail hero. To force existing baked rows
-- to regenerate into the tokenized format, buildMetricsHash gained a
-- PROFILE_FORMAT_VERSION prefix ("v2-price-token").
--
-- That hash is computed in TWO places that MUST match: the TS buildMetricsHash
-- (writes card_profiles.metrics_hash after regeneration) and THIS RPC's
-- current_hash (selects which cards to regenerate). Bumping only the TS side
-- broke parity: existing rows (old hash, no version) would never be reselected,
-- AND any row regenerated by the new TS (hash WITH version) would mismatch this
-- RPC's version-less current_hash forever -> infinite reselection. Prefixing
-- the same 'v2-price-token' literal here restores parity: existing rows mismatch
-- once -> regenerate -> new TS hash matches -> steady state.
--
-- Body is byte-identical to the prior definition EXCEPT the added version
-- prefix line in current_hash. The prefix + '|' join must mirror TS exactly:
--   ["v2-price-token", round0(mp), round0(med7), round0(chg), round0(low),
--    round0(high)].join("|")
--
-- supersedes: 20260625000000_card_profiles_refresh_requeue_low_dollar.sql
...
as $$
  with metrics as (
    select
      ...
      -- Version prefix mirrors PROFILE_FORMAT_VERSION in
      -- lib/ai/card-profile-fallback.ts buildMetricsHash. MUST match exactly.
      left(encode(sha256((
        'v2-price-token' || '|' ||
  ...
  where
    ...
    -- A low-dollar note row whose price has crossed back ABOVE the $2 floor
    -- needs a real LLM summary again. Price-based so it catches every upward
    -- crosser (incl. $1.60 -> $2.40, where the whole-dollar hash bucket is
    -- unchanged). Rows still <= $2 are NOT matched here, so they age out
