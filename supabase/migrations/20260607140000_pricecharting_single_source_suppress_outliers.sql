-- FOLLOW-UP to #205 (20260607130000): suppress outliers in the single-source
-- branch. That migration's PRICECHARTING_PRIMARY high-sample branch surfaced a
-- price ahead of the raw_market_price_outlier suppression (an outlier whose
-- Scrydex price is dropped from the guardrail can classify PRICECHARTING_PRIMARY).
-- The price / price_as_of / provider-compare branches now carry
-- `and not coalesce(raw_market_price_outlier, false)`; downstream confidence /
-- blend / display / provenance blocks cascade via the now-null price. Otherwise
-- identical to 20260607130000.
--
-- == Original intent (unchanged from #205) ==
...
--
-- supersedes: 20260607130000_surface_pricecharting_single_source_display.sql
--             (the LIVE latest definer of public_card_metrics — the #205 merge.
--              This migration is that body verbatim PLUS the not-outlier guard on
--              the single-source price / price_as_of / provider-compare branches.
--              No column added, dropped, reordered or repurposed in the final
--              SELECT.)
--
