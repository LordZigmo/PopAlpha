...
as $$
  ...
  raw_daily_coverage as (
    ...
    from raw_cards rc
    join public.public_price_history pph
      on pph.canonical_slug = rc.canonical_slug
    join bounds b
      on pph.ts >= b.first_day_start_utc
     and pph.ts < b.next_day_start_utc
    group by rc.canonical_slug, (pph.ts at time zone 'UTC')::date
  ),
  window_coverage as (
    select
      nw.window_days,
  ...
  fresh_counts as (
    select
      nw.window_days,
      count(*) filter (where coalesce(wc.covered_days, 0) >= nw.window_days)::bigint as fresh_canonical_raw
    from normalized_windows nw
    left join raw_cards rc on true
    left join window_coverage wc
      on wc.window_days = nw.window_days
     and wc.canonical_slug = rc.canonical_slug
