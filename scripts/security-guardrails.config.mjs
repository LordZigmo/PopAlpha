...
export const INTERNAL_ROUTE_TRUST_CONTRACTS = {
  ...
  "admin/card-profile-coverage": adminSecretRoute("read-only operator dashboard for card_profiles fallback drain (totals + recency + top failure_reason buckets)"),
  ...
  "cron/refresh-jp-price-display": cronSecretRoute("cron/internal automation"),
...
export const OPERATIONAL_SCRIPT_TRUST_CONTRACTS = {
  ...
  "scripts/build-set-pair-map.mjs": operationalScript({
    classification: "service_role_backfill",
    executionMode: "manual_backfill",
    intendedCaller: "trusted operator seeding public.set_pair_map with EN<->JP set pair candidates via canonical_cards + card_printings overlap",
    requiredTrustInputs: ["SUPABASE_SERVICE_ROLE_KEY"],
    expectedSignals: ["service_role_client"],
    usesServiceRole: true,
    notes: "Reads canonical_cards + card_printings + existing set_pair_map via service role, preserves manual rows, and upserts auto rows on en_set_code. Idempotent.",
  }),
  ...
  "scripts/backfill-unpriced-sets.mjs": operationalScript({
    ...
    executionMode: "manual_repair",
    intendedCaller: "trusted operator repairing missing price history via retired debug routes plus direct DB reads",
    requiredTrustInputs: ["CRON_SECRET", "SUPABASE_SERVICE_ROLE_KEY"],
    expectedSignals: ["cron_secret_route_driver", "service_role_client"],
    usesServiceRole: true,
    status: "deprecated",
    shouldStayScript: false,
    notes: "Legacy JustTCG debug-route driver; route entrypoints were removed when English pricing moved fully to Scrydex.",
  }),
  ...
  "scripts/phase3b-remap.mjs": operationalScript({
    classification: "service_role_backfill",
    executionMode: "manual_backfill",
    intendedCaller: "trusted operator driving public.phase3b_remap_batch to completion against _phase3b_refs",
    requiredTrustInputs: ["SUPABASE_SERVICE_ROLE_KEY"],
    expectedSignals: ["service_role_client"],
    usesServiceRole: true,
    notes: "Idempotent + resumable via remapped_at IS NULL.",
  ...
  "scripts/repair-broken-card-images.mjs": operationalScript({
    classification: "service_role_repair",
    executionMode: "manual_repair",
    intendedCaller: "trusted operator repairing card_printings rows whose stored image URL 404s on pokemontcg.io by swapping in Scrydex URLs and mirroring the images into our own Storage bucket",
    requiredTrustInputs: ["SUPABASE_SERVICE_ROLE_KEY", "SCRYDEX_API_KEY"],
    expectedSignals: ["service_role_client"],
    usesServiceRole: true,
  }),
  ...
  "scripts/dump-catalog-pool.mjs": operationalScript({
    classification: "service_role_diagnostic",
    executionMode: "manual_diagnostic",
    intendedCaller: "trusted operator building the candidate-pool JSON (~3-5k slugs + mirrored image URLs) that the offline hard-negative miner consumes to confuse-test a Stage D fine-tune; pairs with scripts/export-finetune-dataset.mjs",
    requiredTrustInputs: ["SUPABASE_SERVICE_ROLE_KEY"],
    expectedSignals: ["service_role_client"],
    usesServiceRole: true,
    notes: "Read-only SELECTs against canonical_cards and scan_eval_runs. Builds a deduped pool from train+val slugs + name siblings + set mates + recent-eval lighthouses + a deterministic random sample. Writes JSON to a local out path; never mutates DB or Storage.",
  }),
  ...
  "scripts/stress-reliability-audit.mjs": operationalScript({
    classification: "manual_cron_route_driver",
    executionMode: "manual_diagnostic",
    intendedCaller: "trusted operator running a read-only reliability scorecard for homepage rails + Pokémon pricing freshness; SQL via `supabase db query --linked` (CLI session auth) and an optional CRON_SECRET HTTP probe against /api/debug/pipeline-health",
    requiredTrustInputs: ["supabase_linked_cli", "CRON_SECRET"],
    expectedSignals: ["cron_secret_route_driver"],
    usesServiceRole: false,
    notes: "Read-only: no INSERT/UPDATE/DELETE, no cron triggers. SQL is executed by the operator's linked-Supabase session and capped at a 15s statement_timeout. The HTTP probe targets /api/debug/pipeline-health (registered as debugCronRoute) — does not invoke privileged write routes. Has a --rehearsal mode that EXPLAINs every probe instead of executing it.",
  }),
  "scripts/sweep-justtcg-finish-repair.mjs": operationalScript({
    ...
    status: "deprecated",
    shouldStayScript: false,
    notes: "Legacy JustTCG repair sweep; route entrypoints were removed when English pricing moved fully to Scrydex.",
  }),
