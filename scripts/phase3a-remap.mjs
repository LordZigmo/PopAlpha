/**
 * One-shot: drive public.phase3a_remap_batch() until _phase3a_refs is
...
while (true) {
  batch++;
  const t0 = Date.now();
  const { data, error } = await supabase.rpc("phase3a_remap_batch", { p_batch_size: 100 });
...
console.log("phase3a remap complete");
