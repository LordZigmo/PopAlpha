 ...
 *
 * Keeps the active scanner image embedder warm so user scans don't pay
 * cold-start latency (5-10s instead of 1-3s). Hosted embedders can
 * spin down idle containers after ~5-10 min; we fire one no-op
 * embedding every 4 min so the warm window never closes.
 *
 * Cost: ~15 calls/hour × 24 × 30 ≈ 10,800/mo × ~$0.0003 ≈ $3/mo.
 * Cheap enough that the zero-cold-start UX is worth it unconditionally.
 *
 * Skip conditions:
 *   - If the active embedder's env vars aren't set, early-exit clean.
 ...
 *
 * Worth retiring if/when the active embedder becomes always-on.
 */
...
export async function GET(req: Request) {
  ...
  if (!hasImageEmbedderConfig()) {
    return NextResponse.json(
      { ok: true, skipped: "active_embedder_not_configured" },
      { status: 200 },
    );
  }

  // Pick any mirrored image as the warmup probe. We don't care which —
  // its only job is to make the active embedder keep its container
  // loaded.
